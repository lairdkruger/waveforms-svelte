 ...
 * BezierEasing - use bezier ease for transition easing function
 * by Gaëtan Renaudeau 2014 - 2015 – MIT License
 */

// Curves
export type Ease = 'linear' | 'in' | 'out'
export type EaseValues = [number, number, number, number]

export function getBezierValues(ease: Ease): EaseValues {
	if (ease === 'linear') return [0.5, 0.5, 0.5, 0.5]
	if (ease === 'in') return [0.55, 0, 1, 0.45] // in
	if (ease === 'out') return [0, 0.55, 0.45, 1] // easeOutCirc
