export interface SmokeMaterialUniforms {
	time: { value: number }
	lowFreqAmp: { value: number }
	midFreqAmp: { value: number }
	highFreqAmp: { value: number }
}
...
export const smokeVertexShader = /* glsl */ `
	...
	void main() {
		 ...
		 noiseMF = midFreqAmp * -1.10 * turbulence( .5 * normal + time );
		...
		//  vec3 positionTest = position + (normalize(normal) * noiseMF);
...
export const smokeFragmentShader = /* glsl */ `
	...
	void main() {
		float disp = 1.0 * 2.0 * noiseMF - (noiseMF * 1.0);
